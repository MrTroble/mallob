...
int getUpperBoundOnResolvents(const std::vector<int>& clauses) {

    tsl::robin_map<int, int> nbOccs; 
    for (int l : clauses) if (l != 0) nbOccs[l]++;

    int nbResolvents = 0;
    for (auto& [l, occs] : nbOccs) if (l>0) {
        nbResolvents += occs * nbOccs[-l];
    }
    return nbResolvents;
}
...
void test() {
    ...
    assert(store.getSelection(100, 100).size() == 30);
...
void test_BZ2File_write_11cnf() {
    std::vector<int> clauses = {
        ...
        111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 0, 
        ...
        155, 156, 157, 158, 159, 160, 161, 162, 163, 164, 165, 0, 
        166, 167, 168, 169, 170, 171, 172, 173, 174, 175, 176, 0, 
        -34, -89, 0, 
        -35, -90, 0, 
        -36, -91, 0, 
        -37, -92, 0, 
    ...
    for (int limit : {100, 200, 400, 800, 1600, 3600}) {
        auto sel = store.getSelection(limit);
        LOG(V2_INFO, "SELECT(%i) = %s (<= %i resolvents)\n", limit, StringUtils::getSummary(
            sel
        ).c_str(), getUpperBoundOnResolvents(sel));
        assert(sel.size() >= 0.9 * limit);
        assert(sel.size() <= limit);
    }
}
